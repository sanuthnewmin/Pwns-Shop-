...
public class PlaceOrder {
    ...
    private static void createOrderUI() {
        ...
        deleteOrderButton.setBackground(new Color(255, 102, 102));
    ...
    private static void deleteOrder() {
        String orderIdStr = JOptionPane.showInputDialog(orderFrame, "Enter the Order ID to delete:", "Delete Order", JOptionPane.PLAIN_MESSAGE);

        if (orderIdStr == null || orderIdStr.trim().isEmpty()) {
            JOptionPane.showMessageDialog(orderFrame, "Order ID cannot be empty.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            ...
            JOptionPane.showMessageDialog(orderFrame, "Order ID must be a valid number.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            try (PreparedStatement orderStmt = conn.prepareStatement(deleteOrderQuery)) {
                ...
                if (rowsAffected > 0) {
                    // Also delete from the OrderDetails and OrderSummary tables
                    String deleteOrderDetailsQuery = "DELETE FROM orderdetails WHERE order_id = ?";
                    try (PreparedStatement detailsStmt = conn.prepareStatement(deleteOrderDetailsQuery)) {
                        detailsStmt.setInt(1, orderId);
                        detailsStmt.executeUpdate();
                    }

                    String deleteOrderSummaryQuery = "DELETE FROM ordersummary WHERE order_id = ?";
                    try (PreparedStatement summaryStmt = conn.prepareStatement(deleteOrderSummaryQuery)) {
                    ...
                    JOptionPane.showMessageDialog(orderFrame, "Order ID not found.", "Error", JOptionPane.ERROR_MESSAGE);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(orderFrame, "Error deleting the order.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            try (Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(query)) {
                ...
                while (rs.next()) {
                    ...
                    orderTableModel.addRow(new Object[]{orderId, customerName, productName , quantity, totalAmount, paymentStatus});
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
